...
static void register_upciedev_proc_new(int num, const char * dfn, struct pciedev_dev     *p_upcie_dev)
{
    char prc_entr[32];
    snprintf(prc_entr, 32, "%ss%i", dfn, num);
...
int GetSlotNumber_exp(struct pci_dev *a_pPciDev, int a_parent_num) // This function should be improved
{
    int pcie_cap;
    u32 tmp_slot_cap = 0;
...
EXPORT_SYMBOL(GetSlotNumber_exp);
...
int pciedev_remove_single_device_exp(struct pci_dev* a_dev, const pciedev_cdev* a_pciedev_cdev_m, const char* a_dev_name, int* brd_num_p)
{
    ...
    unregister_upciedev_proc_new(tmp_slot_num, a_dev_name);
                                    ...
                                    int a_tmp_parent_num)
{
    ...
    a_pciedev->swap         = 0;
    tmp_slot_num = GetSlotNumber_exp(a_dev,a_tmp_parent_num);  // new method added
   ...
   printk(KERN_ALERT "MSI COUNT %i\n", tmp_msi_num);
    ...
    register_upciedev_proc_new(tmp_slot_num, a_dev_name, a_pciedev);
